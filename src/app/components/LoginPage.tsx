...
import { ArrowRight, CalendarDays, Eye, EyeOff, Lock, Mail, MapPin, Sparkles, Users } from 'lucide-react';
...
export function LoginPage({ onLogin, onSignupClick }: LoginPageProps) {
  ...
  const submit = (event: React.FormEvent) => { event.preventDefault(); if (email && password) onLogin(); };
    ...
    <main className="min-h-screen bg-[#F4F6FA] p-3 sm:p-5 lg:p-7">
      <div className="mx-auto grid min-h-[calc(100vh-24px)] max-w-[1480px] overflow-hidden rounded-[28px] bg-white shadow-2xl shadow-slate-900/10 sm:min-h-[calc(100vh-40px)] lg:grid-cols-[1.1fr_.9fr] lg:min-h-[calc(100vh-56px)]">
        <section className="relative hidden overflow-hidden bg-[#101828] p-10 text-white lg:flex lg:flex-col lg:justify-between xl:p-14">
          ...
          <div className="relative flex items-center gap-3"><BrandMark /><span className="text-lg font-semibold tracking-tight">MoEasy</span></div>
        ...
        <section className="flex items-center justify-center px-5 py-10 sm:px-10 lg:px-14 xl:px-24">
          <div className="w-full max-w-[430px]">
            <div className="mb-12 flex items-center gap-3 lg:hidden"><BrandMark /><span className="text-lg font-semibold tracking-tight">MoEasy</span></div>
            ...
            <div className="mt-9 grid gap-3 sm:grid-cols-2">
              <button onClick={onLogin} className="flex items-center justify-center gap-2 rounded-xl bg-[#FEE500] px-4 py-3.5 text-sm font-semibold text-[#181600] transition hover:-translate-y-0.5"><span className="flex h-5 w-5 items-center justify-center rounded-full bg-black text-[10px] text-[#FEE500]">K</span>카카오</button>
              <button onClick={onLogin} className="flex items-center justify-center gap-2 rounded-xl bg-white px-4 py-3.5 text-sm font-semibold ring-1 ring-black/10 transition hover:-translate-y-0.5"><span className="font-bold text-[#4285F4]">G</span>Google</button>
            </div>
...
function BrandMark() { return <span className="flex h-10 w-10 items-center justify-center rounded-[13px] bg-primary text-white shadow-lg shadow-blue-500/25"><Sparkles className="h-5 w-5" /></span>; }
