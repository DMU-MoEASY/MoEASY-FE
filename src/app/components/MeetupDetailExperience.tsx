...
import { ArrowLeft, ArrowRight, CalendarDays, Camera, Check, Clock3, Heart, MapPin, MessageCircle, MoreHorizontal, Plus, Receipt, Send, Settings2, Share2, Users, Wallet } from 'lucide-react';

type Meetup = { name: string; region: string; description: string; members: number; category: string };
type ClubSchedule = { id: number; date: string; title: string; time: string; place: string; attending: boolean };
...
const initialSchedules: ClubSchedule[] = [
  { id: 1, date: '9월 9일 화요일', title: '주간 정기 모임', time: '19:30', place: '반포 한강공원', attending: true },
  { id: 2, date: '9월 14일 일요일', title: '주말 특별 모임', time: '07:00', place: '올림픽공원', attending: false },
  { id: 3, date: '9월 20일 토요일', title: '신입 멤버 환영회', time: '18:00', place: '강남역 11번 출구', attending: true },
];
...
export function MeetupDetailExperience(props: MeetupDetailExperienceProps) {
  ...
  const [scheduleDraft, setScheduleDraft] = useState({ title: '', date: '2026-09-21', time: '19:00', place: '' });
  ...
  const addSchedule = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setSchedules(current => [...current, { id: Date.now(), date: `${date.getMonth() + 1}월 ${date.getDate()}일 ${weekday}요일`, title: scheduleDraft.title.trim(), time: scheduleDraft.time, place: scheduleDraft.place.trim(), attending: true }]);
    setScheduleDraft({ title: '', date: '2026-09-21', time: '19:00', place: '' });
  ...
  return <div className="min-h-screen bg-background">
    ...
    <div className="sticky top-16 z-40 border-b border-border bg-background/90 backdrop-blur-xl"><div className="mx-auto flex max-w-7xl items-center justify-between px-4 sm:px-6"><nav className="flex gap-1 overflow-x-auto scrollbar-hide">{tabs.map(tab => <button key={tab} onClick={() => onTabChange(tab)} className={`relative px-4 py-5 text-sm ${activeTab === tab ? 'font-semibold text-foreground' : 'text-muted-foreground'}`}>{tab}{activeTab === tab && <span className="absolute inset-x-3 bottom-0 h-0.5 rounded-full bg-primary" />}</button>)}</nav><button onClick={onChat} className="hidden items-center gap-2 rounded-full bg-primary px-5 py-2.5 text-sm font-semibold text-white sm:flex"><MessageCircle className="h-4 w-4" />그룹 채팅</button></div></div>

    <main className="mx-auto grid max-w-7xl gap-8 px-4 py-8 pb-28 sm:px-6 lg:grid-cols-[minmax(0,1fr)_340px] lg:py-12">
      <div className="space-y-8">
        {activeTab === '홈' && <HomePanel meetup={meetup} schedules={schedules} onOpenSchedules={() => onTabChange('일정')} onSchedule={onSchedule} onGallery={onGallery} />}
        {activeTab === '게시판' && <BoardPanel posts={posts} draft={postDraft} onDraftChange={setPostDraft} onSubmit={addPost} onToggleLike={(id) => setPosts(current => current.map(post => post.id === id ? {...post, liked: !post.liked, likes: post.likes + (post.liked ? -1 : 1)} : post))} />}
        {activeTab === '일정' && <SchedulePanel schedules={schedules} showForm={showScheduleForm} draft={scheduleDraft} onDraftChange={setScheduleDraft} onToggleForm={() => setShowScheduleForm(value => !value)} onSubmit={addSchedule} onToggleAttendance={(id) => setSchedules(current => current.map(item => item.id === id ? {...item, attending: !item.attending} : item))} onCoordinate={onSchedule} />}
        {activeTab === '회비' && <FinancePanel paid={duesPaid} onTogglePaid={() => setDuesPaid(value => !value)} onOpenFinance={onFinance} onReceipt={onReceipt} />}
      </div>
      <aside className="space-y-4"><div className="rounded-[24px] bg-card p-5 ring-1 ring-black/[0.06]"><p className="text-xs font-semibold text-primary">NEXT MEETUP</p><h3 className="mt-3 text-lg font-semibold">{schedules[0]?.date}</h3><p className="mt-1 text-sm text-muted-foreground">{schedules[0]?.time} · {schedules[0]?.place}</p><button onClick={() => onTabChange('일정')} className="mt-5 w-full rounded-xl bg-[#101828] py-3 text-sm font-semibold text-white">일정 자세히 보기</button></div><div className="rounded-[24px] bg-card p-5 ring-1 ring-black/[0.06]"><div className="flex items-center justify-between"><h3 className="font-semibold">운영진 도구</h3><Settings2 className="h-4 w-4 text-muted-foreground" /></div><div className="mt-3 space-y-1"><ToolButton icon={<Users />} label="가입 신청 3건" onClick={onRequests}/><ToolButton icon={<Users />} label="멤버 관리" onClick={onMembers}/><ToolButton icon={<Wallet />} label="회비 관리" onClick={onFinance}/><ToolButton icon={<Receipt />} label="정산 등록" onClick={onReceipt}/></div></div></aside>
    </main>
...
function SchedulePanel({ schedules, showForm, draft, onDraftChange, onToggleForm, onSubmit, onToggleAttendance, onCoordinate }: { schedules: ClubSchedule[]; showForm: boolean; draft: {title:string;date:string;time:string;place:string}; onDraftChange: (value: {title:string;date:string;time:string;place:string}) => void; onToggleForm: () => void; onSubmit: (event: React.FormEvent<HTMLFormElement>) => void; onToggleAttendance: (id:number) => void; onCoordinate: () => void }) {
  return <section><div className="mb-5 flex items-end justify-between"><div><p className="text-xs font-semibold text-primary">SCHEDULE</p><h2 className="mt-1 text-2xl font-semibold">모임 일정</h2></div><button onClick={onToggleForm} className="flex items-center gap-2 rounded-full bg-[#101828] px-4 py-2.5 text-sm text-white"><Plus className="h-4 w-4"/>일정 추가</button></div>{showForm && <form onSubmit={onSubmit} className="mb-4 grid gap-4 rounded-[22px] bg-card p-5 ring-1 ring-black/[0.06] sm:grid-cols-2"><DetailField label="일정 이름"><input required value={draft.title} onChange={event=>onDraftChange({...draft,title:event.target.value})} className="form-input" placeholder="일정 이름"/></DetailField><DetailField label="장소"><input required value={draft.place} onChange={event=>onDraftChange({...draft,place:event.target.value})} className="form-input" placeholder="모임 장소"/></DetailField><DetailField label="날짜"><input required type="date" value={draft.date} onChange={event=>onDraftChange({...draft,date:event.target.value})} className="form-input"/></DetailField><DetailField label="시간"><input required type="time" value={draft.time} onChange={event=>onDraftChange({...draft,time:event.target.value})} className="form-input"/></DetailField><button type="submit" className="rounded-xl bg-primary py-3.5 text-sm font-semibold text-white sm:col-span-2">일정 등록하기</button></form>}<div className="space-y-3">{schedules.map(item => <article key={item.id} className="rounded-[22px] bg-card p-5 ring-1 ring-black/[0.06]"><div className="flex flex-col gap-4 sm:flex-row sm:items-center"><span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-accent text-primary"><CalendarDays className="h-5 w-5"/></span><div className="min-w-0 flex-1"><p className="text-xs font-semibold text-primary">{item.date}</p><h3 className="mt-1 font-semibold">{item.title}</h3><div className="mt-2 flex flex-wrap gap-3 text-xs text-muted-foreground"><span className="flex items-center gap-1"><Clock3 className="h-3.5 w-3.5"/>{item.time}</span><span className="flex items-center gap-1"><MapPin className="h-3.5 w-3.5"/>{item.place}</span></div></div><button type="button" aria-pressed={item.attending} onClick={() => onToggleAttendance(item.id)} className={`rounded-full px-4 py-2 text-xs font-medium ${item.attending ? 'bg-emerald-50 text-emerald-700' : 'bg-secondary text-muted-foreground'}`}>{item.attending ? <span className="flex items-center gap-1"><Check className="h-3.5 w-3.5"/>참석</span> : '미정'}</button></div></article>)}</div><button onClick={onCoordinate} className="mt-4 w-full rounded-xl bg-[#E8EEFF] py-4 text-sm font-semibold text-primary">멤버들과 가능 시간 조율하기</button></section>;
}
