...
interface ProfileExperienceProps { onLogout: () => void; meetups: Meetup[]; onSelectMeetup: (meetup: Meetup) => void; }
type Profile = { name: string; bio: string; interests: string[] };
const interestOptions = ['러닝', '개발', '등산', '독서', '사진', '맛집'];

export function ProfileExperience({ onLogout, meetups, onSelectMeetup }: ProfileExperienceProps) {
  const [profile, setProfile] = useState<Profile>({ name: '김모이지', bio: '건강한 러닝으로 하루를 시작해요.', interests: ['러닝', '개발', '등산'] });
  const [draft, setDraft] = useState(profile);
  const [isEditing, setIsEditing] = useState(false);
  const openEditor = () => { setDraft(profile); setIsEditing(true); };
  const saveProfile = (event: React.FormEvent<HTMLFormElement>) => { event.preventDefault(); setProfile({ ...draft, name: draft.name.trim(), bio: draft.bio.trim() }); setIsEditing(false); };
  ...
  return <div className="space-y-8 lg:space-y-10">
    <section className="relative overflow-hidden rounded-[28px] bg-[#101828] p-7 text-white sm:p-10">
      ...
      <div className="relative flex flex-col gap-7 sm:flex-row sm:items-end sm:justify-between">
        <div className="flex items-center gap-5"><div className="flex h-24 w-24 items-center justify-center rounded-[28px] bg-gradient-to-br from-blue-400 to-indigo-600 text-2xl font-semibold shadow-xl shadow-black/20">{profile.name.slice(0, 2)}</div><div><span className="rounded-full bg-[#C9FF5C] px-3 py-1 text-xs font-semibold text-[#193300]">매너온도 36.5°</span><h1 className="mt-3 text-3xl font-semibold tracking-[-0.04em]">{profile.name}</h1><p className="mt-1 text-sm text-slate-400">{profile.bio}</p><div className="mt-3 flex flex-wrap gap-1.5">{profile.interests.map(item => <span key={item} className="rounded-full bg-white/10 px-2.5 py-1 text-[11px] text-slate-300">#{item}</span>)}</div></div></div>
